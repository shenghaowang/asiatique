...
def main(config_file):
    ...
    for grid in grids:
        for place_type in place_types:
            logging.debug("Processing grid %s - %s with radius of %s", grid["id"], place_type, radius)
        ...
        counter += 1
        if counter % 100 == 0:
            logging.info("%s city grid processed ... Elapsed time %s seconds",
    ...
    if len(locations) > 0:
        ...
        logging.info("%s target places of interest identified and dumped to %s",
                     len(locations), locations_fp)
